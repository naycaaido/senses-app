...
export default function ReceptionistReservationApiPage() {
  ...
  const load = async () => {
    ...
    try {
      setData(
        (await getReceptionistReservations({ ...filters, limit: 100 })).data,
      );
    } catch (requestError) {
  ...
  return (
    <div className='flex flex-col gap-5'>
      <div className='flex flex-wrap items-center justify-between gap-3'>
        <div>
          <p className='text-xs text-[#434655]'>Pengelolaan reservasi klinik</p>
          <h1 className='text-[28px] font-bold text-[#191c1e]'>Reservasi</h1>
        </div>
      ...
      <Card pad='md'>
        <div className='grid gap-4 sm:grid-cols-3'>
          ...
          <Field label='Email pasien'>
            <Input
              value={filters.email_pasien}
              onChange={(event) =>
                setFilters((current) => ({
                  ...current,
                  email_pasien: event.target.value,
                }))
              }
                ...
                data.map((reservation) => (
                  ...
                  >
                    ...
                    <td className='px-5 py-4 text-sm'>
                      <p className='font-semibold'>{reservation.patientName}</p>
                      <p className='text-xs text-[#434655]'>
                        {reservation.patientEmail}
                      </p>
                    </td>
                    <td className='px-5 py-4 text-sm'>{reservation.service}</td>
                    <td className='px-5 py-4 text-sm'>{reservation.date}</td>
                    <td className='px-5 py-4 text-sm'>{reservation.time}</td>
                    ...
                    <td className='px-5 py-4'>
                      <Link
                        ...
                        to={`/resepsionis/reservasi/${reservation.id}`}
                      >
                        Detail
                      </Link>
                    </td>
