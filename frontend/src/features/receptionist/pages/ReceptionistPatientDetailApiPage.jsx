...
export default function ReceptionistPatientDetailApiPage() {
  const { id } = useParams();
  ...
  return (
    <div className='flex flex-col gap-5'>
      <div className='flex flex-wrap items-center justify-between gap-3'>
        <div>
          <h1 className='text-[28px] font-bold'>Detail Pasien</h1>
          <p className='text-sm text-[#434655]'>{patient.email}</p>
        </div>
        <Button
          onClick={() =>
            navigate(
              `/resepsionis/data-pasien/${encodeURIComponent(email)}/edit`,
            )
          }
        >
          Edit Pasien
      ...
      <div className='grid gap-5 lg:grid-cols-[280px_1fr]'>
        <Card pad='lg'>
          ...
          <div className='mt-4'>
            ...
            <Row label='Tanggal Lahir' value={patient.birthDate} />
            <Row
              label='Profil'
              value={
                <Chip>
                  {patient.profileComplete ? "Lengkap" : "Belum Lengkap"}
                </Chip>
              }
            />
          </div>
        ...
        <div className='flex flex-col gap-5'>
          <Card pad='md'>
            ...
            <div className='mt-3'>
              ...
              <Row label='Pekerjaan' value={patient.job} />
            </div>
          </Card>
          <Card pad='md'>
            <h2 className='font-bold'>Riwayat Reservasi</h2>
            <div className='mt-3 space-y-3'>
              {history.length === 0 ? (
                <p className='text-sm text-[#434655]'>Belum ada reservasi.</p>
              ) : (
                history.map((item) => (
                  <div
                    key={item.id}
                    className='flex flex-wrap items-center justify-between gap-3 border-b border-[#e6e6e2] pb-3'
                  >
                    <div>
                    ...
                    <div className='flex items-center gap-3'>
                      ...
                      <Link
                        to={`/resepsionis/reservasi/${item.id}`}
          ...
          <Card pad='md'>
            ...
            <div className='mt-3 space-y-3'>
              {payments.length === 0 ? (
                <p className='text-sm text-[#434655]'>Belum ada pembayaran.</p>
              ) : (
                payments.map((payment) => (
                  <div
                    key={payment.no_reservasi}
                    className='flex justify-between gap-3 border-b border-[#e6e6e2] pb-3'
                  >
                    <div>
                      <p className='font-semibold'>
                        {payment.reservasi?.layanan?.nama_layanan || "Layanan"}
                      </p>
                      <p className='text-sm text-[#434655]'>
