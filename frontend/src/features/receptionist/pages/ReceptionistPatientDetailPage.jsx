...
function CardHeader({ icon, tone, title }) {
  return (
    <div className="flex items-center gap-3 px-5 py-4">
      <span className={`flex size-8 items-center justify-center rounded-lg ${tone === 'blue' ? 'bg-[#eff6ff] text-[#2563eb]' : tone === 'gold' ? 'bg-[#f6f0e2] text-[#a8945e]' : 'bg-[#ecfdf5] text-[#059669]'}`}>{icon}</span>
...
function Row({ label, value }) {
  return (
    <div className="flex items-center justify-between gap-4 border-b border-[#e6e6e2] py-3 last:border-b-0">
      <span className="text-xs font-medium text-[#434655]">{label}</span>
      <span className="text-right text-[13px] text-[#191c1e]">{value || '—'}</span>
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-end justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-bold text-[#191c1e]">Detail Pasien</h1>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-[280px_1fr] lg:items-start">
        <Card pad="lg" className="text-center">
          ...
          <p className="mt-4 text-xl font-bold text-[#191c1e]">{patient.name}</p>
                ...
                <tbody>
                  ...
                  {history.map((r) => (
                    <tr key={r.id} className="border-b border-[#e6e6e2] last:border-b-0">
                      <td className="px-5 py-4 text-[13px] font-bold text-[#191c1e]">{r.id}</td>
                      <td className="px-5 py-4 text-[13px] font-semibold text-[#191c1e]">{r.service}</td>
