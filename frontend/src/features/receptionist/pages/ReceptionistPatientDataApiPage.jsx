...
export default function ReceptionistPatientDataApiPage() {
  ...
  return (
    <div className='flex flex-col gap-5'>
      <div className='flex flex-wrap items-center justify-between gap-3'>
        <div>
          <p className='text-xs text-[#434655]'>Manajemen data pasien</p>
          <h1 className='text-[28px] font-bold'>Data Pasien</h1>
        </div>
      ...
      <Card pad='md'>
        ...
        />
        <p className='mt-3 text-xs text-[#434655]'>{total} pasien terdaftar</p>
      </Card>
      <Card>
        {error && (
          ...
          <table className='w-full min-w-[700px]'>
            <thead>
              <tr className='border-b bg-[#f5f5f3]/60 text-left'>
                {["Pasien", "Telepon", "Email", "Kota", "Aksi"].map((label) => (
            ...
            <tbody>
              {loading ? (
                ...
                patients.map((patient) => (
                  <tr key={patient.email} className='border-b border-[#e6e6e2]'>
                    ...
                    <td className='px-5 py-4 text-sm'>{patient.email}</td>
                    <td className='px-5 py-4 text-sm'>{patient.city || "—"}</td>
                    <td className='px-5 py-4'>
                      <Button
                        variant='outline'
                        onClick={() =>
                          navigate(
                            `/resepsionis/data-pasien/${encodeURIComponent(patient.email)}`,
                          )
                        }
                      >
                        Lihat
