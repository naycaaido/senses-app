  ...
  const activeReservation = useMemo(
    () =>
      reservations
        ...
        .sort((left, right) => scheduledAt(left) - scheduledAt(right))[0] ||
      null,
    [reservations],
  );
  ...
  if (loading) {
    return (
      <div className='mx-auto max-w-[1200px] px-4 pb-8 pt-4 md:px-6 md:pb-12 md:pt-6'>
        ...
        <p
          className='rounded-2xl border border-[#f0ede7] bg-white px-6 py-12 text-center text-[15px] text-[#6b6b6b]'
          role='status'
        >
  ...
  if (error) {
    return (
      <div className='mx-auto max-w-[1200px] px-4 pb-8 pt-4 md:px-6 md:pb-12 md:pt-6'>
        <Header firstName={firstName} />
        <div
          className='rounded-2xl border border-[#e5c7c1] bg-[#fff5f2] px-6 py-8 text-center text-[#8a3324]'
          role='alert'
        >
  ...
  return (
    <div className='mx-auto max-w-[1200px] px-4 pb-8 pt-4 md:px-6 md:pb-12 md:pt-6'>
      ...
      <div className='flex flex-col  items-stretch gap-6 min-[1025px]:items-start min-[1025px]:flex-row'>
        <aside className='w-full shrink-0 overflow-hidden rounded-2xl bg-[#3d4940] shadow-[0_1px_3px_rgba(44,44,44,0.06),0_12px_32px_-16px_rgba(61,73,64,0.22)] min-[1025px]:w-[360px] h-[300px]'>
          <div className='flex flex-col items-center gap-3 px-6 pb-7 pt-8 text-center'>
            <p className='m-0 text-xs font-semibold uppercase tracking-[0.18em] text-[#fbf8f3]/70'>
              Reservasi Aktif
            </p>
            <p className='m-0 font-serif text-[64px] font-bold leading-[64px] text-[#fbf8f3] min-[1025px]:text-[88px] min-[1025px]:leading-[88px]'>
              {time}
            </p>
        ...
        <section className='flex min-w-0 flex-1 flex-col gap-4'>
          <div className='rounded-2xl border border-[#f0ede7] bg-white p-5 md:p-[25px]'>
            <h2 className='mb-5 font-serif text-[22px] font-bold leading-[28.6px] text-[#3d4940]'>
              Reservasi Aktif
            </h2>
            <div className='grid grid-cols-1 gap-5 md:grid-cols-2'>
              {[
                ...
                <div key={label} className='flex items-start gap-3'>
                  ...
                  <div>
                    <p className='m-0 text-xs font-semibold uppercase leading-4 tracking-[0.025em] text-[#6b6b6b]'>
                      {label}
                    </p>
          ...
          <Link
            to='/pasien/riwayat'
            className='flex items-center justify-between rounded-2xl border border-[#f0ede7] bg-white p-[21px] hover:bg-[#faf7f2]'
          >
            <div>
              <p className='m-0 font-serif text-base font-bold leading-[26.4px] text-[#3d4940]'>
                Riwayat Kunjungan
              </p>
